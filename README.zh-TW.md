...
被扛住的 session 回來時叫做 **執行中（無人看著）** 而不是「已關閉」，而且是
在第一次繪製之前用 `tmux has-session` 問過的，不是丟給背景執行緒：一個過一下
才自己修正的狀態，在一個唯一職責就是「一眼可信」的表面上是閃爍。它的圓點用
中性色，因為那個 agent 的 hooks 還對著上一個 app 實例握著的 port。「它在跑」
我們知道，「它現在在做什麼」要打開才知道。

尚未做：WSL 與 SSH 世界的同一件事，以及斷開狀態下的即時狀態。
...
| 二・session 即時狀態 | 執行中 / ⚠等你授權 / ⚠等你回覆 / ⚠等你確認資料夾 / 待命 / 執行中（無人看著）/ 結束 | hook 回報 |
...
- 「活得比 app 久」目前只有本機。repo 在 WSL distro 或 SSH host 上的卡片，
  app 一關還是會停，要按繼續；而被扛過一次重啟的 session 在打開之前顯示為
  **執行中（無人看著）**，因為它的 hooks 還在對一個已經不存在的 port 回報
