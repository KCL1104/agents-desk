...
新增 `Status::Detached`,啟動時在第一次繪製之前對每個存下來的 session
問一次 `tmux has-session`——放在繪製前而不是丟背景執行緒,是因為一個過一下才自己
修正的狀態,在一個唯一職責就是「一眼可信」的表面上是閃爍。

**這個謊言原本會講兩次。** 狀態標籤講一次(側欄 / Overview / ⌘K 面板),
卡片再用 `live.stopped`(「未執行」)講一次——兩條路徑,同一個假話。
兩處都修了。

三個刻意的判定:

- **圓點用中性色,不是 accent。** agent 在跑沒錯,但它的 hooks 還對著
  **上一個 app 實例的 port**,那個 port 已經不在了——所以「它在跑」我們知道,
  「它現在在做什麼」我們不知道。用執行中的實心圓點會是在宣稱我們沒有的知識。
- **分組進「開發中」而不是「完成」**,否則等於在分組裡把那句謊言再講一次。
- **`live` 維持 false**:這個行程裡沒有 pty 扛著它。打開卡片才 attach。

### 仍然沒做

- **SSH 與 WSL**:見上面的核心判定,各自要單獨決策。
- **detached session 的即時狀態**:接回之前讀不到,因為 hook URL 指向舊 port。
  要真的即時,得讓 hook 的埠位穩定或讓 agent 重新學到新位址——那是另一個題目。
  現階段的誠實作法是承認不知道,而不是猜。
