...
| 5b | ~~卡片狀態要文字備援~~(看 Xirp 的 `◎ idle` 推論的缺口) | **已經有了**:`liveLabel()` 讓卡片、Overview、側欄三處都印出狀態字,且都進 aria-label。AgentDesk 是**色 + 形 + 字**,Xirp 是形 + 字 | — | **不用做** |
...
2. **Rules + Skills**(配對 #1、#2、#23)。**已實作,但落點與 Xirp 不同。**
   Xirp 把它做成專案的一個分頁;AgentDesk 沒有專案實體,而且更重要的是,
...
11. **門檻 vs 密度**(配對 #29)。**已拍板:不做空首頁,改縮表單 + ⌘K 建卡;已實作。**
    ...
    AgentDesk 的首頁是一張四欄看板。兩者都合理,但**低門檻正是審查判定最弱的地方**。
...
- **本研究最該記住的方法教訓**:對照競品的截圖推論「我方缺什麼」,有三次推錯
  ...
  三項 AgentDesk 都已經有,只是我沒有先查自己的程式碼就下判斷。
