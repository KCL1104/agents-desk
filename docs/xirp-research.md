...
**分歧:Marol 的 positioning 回答 how(終端是真的),Xirp 回答 why(為什麼需要一個 app)。**
他們的答案(接企業 catalog)Marol 抄不了也不該抄,但問題是真的,
而 Marol 已有一整套本機零件(worktree、checkpoints、transcript、跨 session 訊息)沒被組織成論點。
...
對照 Marol:app 結束 = PTY 結束,重開靠 `claude --continue` 由 cwd 找回對話,scrollback 不落盤。
...
### 5.1 Projects — 一個 Marol 沒有的實體層

Xirp 有「專案」這個一等實體,Marol 沒有(卡片直接綁路徑)。
...
Marol 缺口:無專案實體、無 Files 分頁、無 Skills/Rules 表面、
Git 側只有 attempt 的 diff(無分支列表、無 commit 歷史、無 commit 控制)。

### 5.2 Sessions — 比 Marol 鬆的耦合,和幾個沒有的動作
...
- **狀態只有四態**:Working / Idle / Waiting / Finished-or-failed。
  (Marol 六態 — running / waiting-permission / waiting-you / awaiting-trust / idle / ended —
  **比他們細**。)
...
Marol 缺口:general session、MRU 切換器、fork 對話、截圖輸入、背景啟動、
終端牆的分組與篩選、中途換 agent。

### 5.3 Settings — Marol 最弱的一環,對手做得很完整

Marol 現況:單一 `EnvPanel.tsx`(536 行)藏在側欄底部一顆 11px 灰字按鈕後,
...
| **Git & worktrees** | worktree **建立位置**與**命名規則**;專案層 setup / cleanup 腳本 | 位置固定 `~/.marol/worktrees/`;腳本有(`.marol/config.json`) |
...
真終端路線的代價是真實存在的,連他們也沒躲掉——這對 Marol 是安慰也是提醒。

### 視覺系統(與 Marol 的紀律正面對照)

- **底色是中性近黑**,不是溫暖的血墨。品牌色是**橘**(logo、`+` 鈕、資料夾圖示、齒輪),
  但場上同時還有:**薄荷綠**(狀態字、計數徽章、Installed)、**青綠**(分支名、分頁底線)、
  **紫**(worktree 圖示、對話框主要按鈕)、**紅**(Close)。
- 也就是**五個顏色同時在講話**。Marol 的「一顆寶石規則」在這裡是明確更嚴的紀律,
  不需要向他們看齊——這是少數 Marol 的設計系統贏面清楚的地方。
...
**這跟 Marol 的操作台密度是兩個極端。** 他們的大門是一個文字框(「說出你要出什麼貨」),
Marol 的大門是一張看板。兩者都合理,但值得意識到:
prompt-first 的門檻明顯更低,而**低門檻正是前次審查判定 Marol 最弱的地方**。
...
**狀態用「字」不用「色邊」**,這與 Marol 的 3px 狀態邊 + 7px 圓點是不同的語言。
他們的四態靠讀,Marol 的六態靠掃——Marol 的更適合牆面掃視,但**沒有文字備援**。
...
正是 Marol `dev-preview.md` 做過的題目,而 Marol 在那份決策裡**否決了注入式 inspect**。
...
**每個動作的快捷鍵就長在按鈕上**。Marol 的手勢目前活在 tooltip 與 ⌘/ 表裡——
前次審查點名過這件事,這是最便宜的修法。
...
**版面密度是一個 25–200% 的乘數**——Marol 的密度是設計決定(13px body、5–10px padding),
...
Marol 的哲學是「每個拒絕都附上完整理由」——但那些理由現在活在 README 與 `docs/decisions/`,
**不在使用者打開設定、找不到東西的那一刻**。這一招幾乎是為 Marol 量身訂做的。

同一頁的 agent 卡片還把機制承諾寫進了選擇的當下:

> **Claude** — 「**Hand the terminal over to** Anthropic's `claude` CLI.」 `Installed`(綠)

一句話講完「這個 app 做的事就是把終端交出去」。Marol 的定位句
...
而且與 Marol「絕不用空白冒充狀態、缺席要有理由」的原則同源。
...
第二輪我說「Rules/Skills 是能讓 codex/gemini 平價的功能」——這個論點對 Marol 仍然成立
...
Marol 的三視圖 + 抽屜目前沒有「回上一個地方」的概念。

---

## 六、模式 → 缺口配對表

| # | Xirp 的做法 | Marol 現況 | 哲學檢查 | 判定 |
|---|---|---|---|---|
...
| 4 | **`Cmd+,` 開設定** | 無標準鍵 | 安全 | **做** |
| 5 | **Prompting 設定:公開 app 自己加了什麼指令** | **查核後大幅縮小**:開 attempt 的對話框本來就顯示完整 prompt 且**可就地編輯**,模板在 `<data_dir>/prompt-template.md` 也可改。缺的只有「設定裡沒有模板的入口」 | 安全 | **做**,但只是一顆按鈕 |
| 5b | ~~卡片狀態要文字備援~~(看 Xirp 的 `◎ idle` 推論的缺口) | **已經有了**:`liveLabel()` 讓卡片、Overview、側欄三處都印出狀態字,且都進 aria-label。Marol 是**色 + 形 + 字**,Xirp 是形 + 字 | — | **不用做** |
| 5c | ~~general session(無專案的 session)~~ | **已經有了**:ad-hoc session 就是——無卡片、無 worktree,直接在任意目錄開 agent。缺口不是「沒有低門檻入口」,是「入口不是門面」 | — | **併入 #29** |
...
| 31 | **每個功能的介面文案極短,深度全部推給導覽與 docs** | 370 個字串裡 69 個 ≥90 字元、21 個 ≥140;同一概念常被教兩次 | 安全,但**不可波及拒絕與錯誤內容** | **做**(見 §7.1) |
...
> 進度:**七項全部實作完成**(⌘, · 搜尋 · 七個分區 · 四張拒絕說明卡 ·
> 開場 prompt 模板入口 · 重播導覽 · 第三方授權頁 · ⌘L 回上一個 session ·
> 對話框印和弦 · Rules/Skills)。
>
> 兩個實作時才浮現的判斷:
> **(a) 面板改名為「設定」**——它現在有搜尋與七個分區,診斷只是其中一格;
> 叫「環境」是對自己的低估,而低估自己正是可發現性問題的一部分。
> **(b) 歡迎面板 / 重播導覽 / 說明文件三扇門放在「一般」而不是「進階」**——
> 這三扇門都是給還在摸索的人開的,而摸索的人最不會去點一個叫「進階」的東西。

把「Xirp 的設定頁」當成一份現成規格,對著改 `EnvPanel`:

0. **拒絕說明卡**(配對 #21)。**這是整份研究裡最該先做的一件事,而且最便宜。**
   Marol 的哲學是「每個拒絕都附上完整理由」,但理由現在活在 README 與決策文件裡,
...
0b. **提示文字精簡**(配對 #31)。與 #0 是同一枚硬幣的兩面:
   **字不是變少,是搬到對的地方。** 完整任務規格見 **§7.1**。
1. **`Cmd+,` + 搜尋欄 + 分區**(配對 #3、#4、#27)。搜尋欄不需要重新設計資訊架構,
   就治好「差異化功能全藏在一顆灰字按鈕後」——前次審查判定的最弱項。
   分區可直接借他們的骨架,但 **Projects 區段先跳過**(Marol 無專案層)。
   設定列改成卡片、說明文字裡帶目前值,是同一批工。
2. **Rules + Skills**(配對 #1、#2、#23)。**已實作,但落點與 Xirp 不同。**
   Xirp 把它做成專案的一個分頁;Marol 沒有專案實體,而且更重要的是,
   這個問題**在 review 的時候才最尖銳**——「agent 到底該遵守什麼慣例,它讀到了嗎」。
   所以做成**檢視器的第三格「它知道什麼」**,讀的是 attempt 自己 worktree 的路徑
   (那才是 agent 真正的 cwd,不是 repo 的)。命名也跟著問題走,不跟著檔案類型走。
   原始論證,用**格位模型**:
   ...
   **注意第三輪的補正**:Xirp 自己的 Rules 只列 `CLAUDE.md`、Skills 只認 `.claude/skills/`,
   ...
   對 Marol 仍然成立(`AGENTS.md` 是真的),但**這是 Marol 可以贏過他們的地方,
   不是跟隨他們的地方**。
3. **Prompting 可見化**(配對 #5)。Marol 有 `prompt.rs`,會在 session 開場替使用者組話。
...
> 任務來源:作者實際使用後的觀察——「打開發現太多冗餘的提示文字」。
> 目標形態:像 Xirp 一樣**各功能的介面文案極短**,深度交給初次導覽與 docs。

#### 現況量測(`ui/src/i18n/messages.ts`,en 為準)

- 總共 **370 個字串**;其中 **69 個 ≥90 字元**、**21 個 ≥140 字元**。
- 最長五條:`usage.tip` 323、`attempt.yoloHint` 269、`coach.waiting.body` 247、
  `attempt.unmeasuredHint` 235、`coach.finish.body` 232。
- 常駐介面裡的說明段落(`<p className="muted small">`)集中在:
  `EnvPanel` 6、`NewTaskDialog` 4、`Board` 4、`NewSessionDialog` 2、`StartAttemptDialog` 2。
- `StartAttemptDialog` 一個對話框裡就有三段說明(`acceptHint` / `yoloHint` / `trustHint`),
  其中兩段還帶著 `data-testid`。

#### 診斷:問題不是字太長,是同一個概念被教兩次

文案本身寫得好,而且是產品的聲音。真正的冗餘在於**常駐介面與初次導覽在教同一件事**:

| 概念 | 目前教它的地方 | 該歸誰 |
|---|---|---|
| worktree 是隔離邊界(attempt 只花得起自己的分支) | `newTask.baseHint`、`coach.attempt.body`、`attempt.yoloHint`、`attempt.acceptHint`——**四處** | 導覽教一次;三處縮成半行 |
| 新資料夾會先問信任 | `attempt.trustHint`(190)、`coach.attempt.body`——兩處 | 導覽 + 卡片上那顆旋轉方塊狀態本身就是答案 |
| Ctrl+字母屬於 shell,app 手勢要加 Shift | `keys.shellNote`(206)、`coach.terminal.body`(182)——兩處 | 導覽教一次;⌘/ 表留一行 |
| CLAUDE.md / skills / MCP 是原生載入的 | `newSession.cwdHint`(161)、`newTask.promptHint`(167)——兩處 | **未來的 Rules 分頁(#2)天生就是這個答案** |
| 終局會凍結 diff、收回 worktree | `coach.finish.body`(232)、`inspector.discardHint`、`inspector.frozenHint`——三處 | 導覽 + **armed 按鈕改名時本來就會說** |

最後一列值得特別看:**armed 二擊按鈕在第一次點擊時會把自己改名成「它即將做的事」**——
那已經是這個 app 說明後果的機制,而且說在後果發生的那一刻。
`coach.finish.body` 在教按鈕自己會講的話。

#### 三個歸屬層(每個概念只能有一個家)

1. **初次導覽教一次** — coach marks 的 body **可以留長**,它們一生只出現一次,不是常駐噪音。
   這裡是唯一適合放完整心智模型的地方(卡片→attempt→worktree)。
2. **介面留一行** — 常駐表面只留「這顆按鈕會做什麼」等級的一句,不解釋為什麼。
3. **docs 收全文** — README(en + zh-TW)已經是那份文件,只是**app 裡到不了它**。

#### 絕不刪的清單(這條界線比裁切本身重要)

以下不但不刪,#0 的拒絕說明卡還會讓它們**變長**,而且那是刻意的:

- **拒絕與其完整理由**(DESIGN.md 明訂「refusals 要就地完整換行呈現」)
- **`FriendlyError` 的原文**與所有要求動作的錯誤
- **量測免責**:`attempt.unmeasuredHint`(未量測的 agent 不自動送出)、
  非 Claude 的無狀態訊號 chip、`preview.sshHint`——這些是「誠實優先」的本體
- **armed 按鈕改名後的字**
- **信任閘門**的說明(產品明訂它永不被繞過)
...
- 常駐介面(對話框、抽屜、面板、看板)中 ≥90 字元的字串由 **69 降到 ≤25**,
  且剩下每一條都能歸進「拒絕 / 免責 / 後果」三類之一。
- **沒有任何一個概念在兩個常駐表面各教一次。**
- 每刪掉一條,都要能指出「這件事現在在哪裡學得到」——導覽、docs、或該功能自己的空狀態。
- **app 裡要有一條到得了 docs 的路。** 沒有這條路,「推給 docs」等於把知識丟掉而不是搬家。
  最小作法:設定頁底部一條說明文件連結(對照 Xirp 把版本與更新狀態釘在設定底部)。

#### 實作約束

- i18n 是雙語 typed catalog,**刪 key 要兩語同時刪**,
  `i18n.spec.ts` 的 SHARED 豁免清單要跟著更新,少一邊會 typecheck 失敗。
- `StartAttemptDialog` 的 `accept-hint` / `yolo-hint` 帶 `data-testid`,Playwright 測試會連動。
- **以英文長度為裁切基準**。zh-TW 每字資訊密度高得多,同一條 hint 中文短很多——
  照中文的觀感裁切會把英文砍過頭。
- 這正是 `/impeccable clarify` 的職責範圍(該指令的定位就是「coach marks 與空狀態文案」)。

#### 建議順序

先做**去重**(上表五個概念,把常駐那一份縮掉),再做**裁切**(剩下的長句),
...
6. **持久化 session**(配對 #8)。第一輪列為證據最弱,第二輪證據充分。
   **決策文件已寫:`docs/decisions/persistence.md`(待拍板)。**
   核心判定是「持久化是世界的能力,不是 app 的前提」,v1 只做 local(+WSL 待議),
   SSH 押後;並發現 `Status::Saved` 與 scrollback 的 seq 續傳是已經鋪好的兩塊地基。
   原本要回答的三題:
...
8. **general session(配對 #15)**。這條比看起來重要:
   Xirp 最便宜的第一個動作是「就開一個 agent」,Marol 是「建任務 → 開 attempt → 長 worktree」。
...
9. **規模目標(已拍板:不追;而「把天花板說出來」查證後撤回)**。
   我原本建議「不追規模,但要把天花板說出來」。逐條查證後,**天花板其實已經說出來了**:
   併發上限就顯示在看板上(`running / max` 加 −/＋ 與佇列數)、
   終端牆放不下時是**捲動**而不是把 pane 壓到 TUI 用不了的尺寸(`autoCols` 由寬度推導、
   `gridStyle` 給 rows 最小高度)、WebGL context 跟著可見性自癒。
   剩下唯一沒說的是「你的機器扛不扛得住 N 個 agent」——那是**量不到的東西**,
   照原則 #2 就不該畫成儀表。**所以這項不做**,理由記在這裡。
   原始論證:Xirp 為 50+ 設計(minimap + grid view + 分組 + 篩選 + MRU + 專案層,
   全都是 50 個 session 才需要的東西)。Marol 併發預設 3,介面為 3–10 張卡設計且設計得好。
   **不建議追**,但該回答一個更誠實的問題:Marol 的實際上限是幾張卡,app 有沒有說出來?
...
11. **門檻 vs 密度**(配對 #29)。**已拍板:不做空首頁,改縮表單 + ⌘K 建卡;已實作。**
    ...
    實作時發現「縮表單」真正的問題不是欄位數,是**順序**:對話框第一眼看到的是
    「標題(選填)」——一個可以留白的欄位站在最前面,等於一開口就要人做一個
    不必做的決定。改成目標欄第一、自動聚焦;三個 journey 原本斷言焦點落在標題欄,
    現在斷言落在目標欄,那是更強的主張。

    **摺疊其餘欄位的版本做了又撤掉。** 32 個測試呼叫點要改本身就是訊號
    (那是介面契約在說話),而且摺疊多帶一個風險:預設 repo 被收起來,
    可能在錯的 repo 上開卡。它的收益 ⌘K 那條路已經給了——
    打一句話 → 對話框帶著目標開、其餘全有預設 → ⌘↵。
    **刻意設 12 字元下限**:「auth」是搜尋,不是目標;讓每個查詢都長得像一張卡是噪音。
12. **字級與版面密度是否交給使用者**(配對 #28)。他們給三軌字級(終端 / diff / 檔案樹)
    加一個 25–200% 的密度乘數。Marol 的密度是刻意的設計決定
...
1. **Portal / catalog 綁定**。Marol 沒有企業目錄,不該假造。
...
4. **context-window 百分比**(配對 #20)。Marol 已明確拒絕(沒有誠實的分母),維持拒絕。
...
6. **把 vendor neutrality 當口號**。三個 agent + 要帳號 + macOS only,離 neutral 還有距離。
   Marol 的作法維持:**量得到的才顯示,量不到的戴免責 chip**。

## 九、對外敘事的機會(不點名比較)

- 這個類別現在有大公司背書,「多 agent 桌面控制台」不再需要解釋自己為什麼存在。
- Xirp 的每個限制,剛好是 Marol 已經站著的位置:
  **開源 Apache-2.0 / 無帳號 / 無遙測 / 三平台 + WSL + SSH 三個世界 / 本機優先**。
- 誠實記下 Marol **弱於** Xirp 的地方:設定的可發現性、無情境表面(rules/skills)、
  無檔案瀏覽、無專案層、規模(3 vs 50+)、macOS 以外沒有簽章安裝檔。
- README 值得補一段回答 *why*,但用 Marol 自己的答案:
...
- **本研究最該記住的方法教訓**:對照競品的截圖推論「我方缺什麼」,有三次推錯
  (配對 #5、#5b、#5c)——Prompting 可見化、卡片狀態文字、general session,
  三項 Marol 都已經有,只是我沒有先查自己的程式碼就下判斷。
  **看完對手要先審自己,再命名缺口。** 六個待拍板問題因此縮成三個。
